  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (isSignUp) {
        ...
        if (result?.error) throw new Error(result.error);
        // If login succeeds, the server action calls `redirect('/')`, 
        // which throws a NEXT_REDIRECT error we shouldn't catch entirely, 
        // but NextJS handles it internally. 
      }
    } catch (error: any) {
          ...
          <div className="relative group">
            ...
            <div className="relative bg-card/60 backdrop-blur-xl rounded-2xl p-6 border border-primary/20 shadow-2xl overflow-hidden">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="relative flex items-center overflow-hidden rounded-lg">
                      ...
                      >
                        {showPassword ? (
                          ...
                          <EyeOff className="w-4 h-4 text-foreground/40 hover:text-primary transition-colors duration-300" />
                        )}
                      </div>
                ...
                {/* Divider (Removed Google Auth along with it) */}
